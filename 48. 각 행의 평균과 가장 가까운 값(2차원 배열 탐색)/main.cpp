...
// 1차 복습 풀이시간 : 16분 
// 2차 복습 풀이시간 : 14분 30초 내 (복습 필요 / 예외처리 누락 및 최적화 필요)
...
int main()
{
	...
	*/
	...
	for (int i = 0; i < 9; i++)
	{
		...
		for (int j = 1; j < 9; j++)
		{
			if (abs(arr[i][j] - average[i]) < abs(res - average[i]))
			{
				res = arr[i][j];
			}
			if (abs(arr[i][j] - average[i]) == abs(res - average[i]))
